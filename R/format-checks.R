...
pkgdeps_format <- function (checks, sec_num) {
    ...
    if (nrow (re_exports) > 0L) {
        index <- which (deps$package %in% re_exports$package &
            is.na (deps$ncalls))
        if (length (index) > 0L) {
            deps <- deps [-index, ]
        }
    ...
    deps <- knitr::kable (rbind (internal, deps), row.names = FALSE)
    ...
    deps_rep <- c (
        ...
        paste0 (
            ...
            "both internal (r-base + recommended, along with the pacakge ",
            "itself), and external (imported and suggested packages). ",
            "'NA' values indicate packages to which no identified calls ",
            "to R functions could be found. Note that these results are ",
            "generated by an automated code-tagging system which may not ",
