...
checks](https://github.com/ropensci-review-tools/goodpractice). The `checks` item passed
...
```{r check_file, echo = FALSE, cache = FALSE}
check_file <- fs::path (fs::path_temp (), "checks.Rds")
```


```{r pkgcheck-dummy-data, echo = FALSE, eval = !file.exists (check_file), message = FALSE, warning = FALSE}
...
```{r pkgcheck-str, echo = FALSE, cache = FALSE, warning = FALSE}
print (str (readRDS (check_file), max.level = 3L))
```
...
`R/check-has-citation.R`](https://github.com/ropensci-review-tools/pkgcheck/blob/main/R/check-has-citation.R):
...
The `output_pkgchk_has_scrap()` function looks like this:
...
item. In this case, both `summary` and `print` methods are only triggered `if
...
stored in the corresponding `obj` list item displayed as formatted lists. A
package with "scrap" files, `"a"` and `"b"`, would thus have `out$print$obj <-
