...
The [main `pkgcheck()` function](https://docs.ropensci.org/pkgcheck/reference/pkgcheck.html) has an additional parameter, `extra_env` which specifies,
...
## 4. The order of checks

*This section is for `pkgcheck` developers only - users can not modify the order
in which check results are summarised or printed.*

The order of the `summary` output is determined from [a function called
`order_checks()` in the `R/summarise-checks.R`
...
Any additional checks defined in an `extra_env` will appear *after* all of the
standard checks, and prior to the `R CMD check` results which always appear
last. This order may only be modified by editing the list in that function.
The order of check results in the `print` method is also hard-coded, defined in
the [main `print.pkgcheck`
