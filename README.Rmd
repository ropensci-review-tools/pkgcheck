...
[`pkgstats`](https://github.com/ropenscilabs/pkgstats) package, installation of
which also requires both [`ctags`](https://ctags.io) and [GNU
...
Once `pkgstats` has been successfully installed, the `pkgcheck` package can
...
GitHub markdown only renders them in black-and-white.) The full details of the
object returned from the `pkgcheck` function may be seen with the default
`print` method:
