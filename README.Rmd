...
To avoid potential namespace conflicts with existing CRAN packages, `pkgcheck` includes the helper function `fn_names_on_cran`. You can use this interactively during development to check if your proposed function names are already in use. This is a useful step to perform before committing new functions and running a full `pkgcheck`:
