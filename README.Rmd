...
The package also works by locally caching previously analysed packages. This is
done by default in `tempdir()`, but can be directed to a specified and enduring
location by setting another environmental variable:
```{r cache_dir, eval = FALSE}
Sys.setenv ("PKGREP_CACHE" = "/path/to/cache/directory/")
```
Again, this can be set via `~/.Renviron` to obviate need to set this directory
manually each time.
